...
if(string.IsNullOrEmpty(typesenseUri))
    throw new InvalidOperationException($"Invalid typesense URI: {typesenseUri}");

var typesenseApiKey = builder.Configuration["typesense-api-key"];
if(string.IsNullOrEmpty(typesenseApiKey))
    throw new InvalidOperationException($"Invalid api key: {typesenseApiKey}");
...
app.MapGet("/search", async (string query, ITypesenseClient client) =>
{
    ...
    {
        return Results.Problem("Search not successful",e.Message);
    }
});
...
app.MapGet("/search/similar-titles", async (string query, ITypesenseClient client) =>
{
    ...
    {
        return Results.Problem("Search not successful for title", e.Message);
    }
});
