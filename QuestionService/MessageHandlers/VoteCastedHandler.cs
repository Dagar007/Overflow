...
public class VoteCastedHandler(QuestionDbContext db)
{
    public async Task Handle(VoteCasted message)
    {
        ...
        {
            await db.Questions.Where(x => x.Id == message.TargetId)
                .ExecuteUpdateAsync(setter => 
                    setter.SetProperty(x => x.Votes, x => x.Votes + message.VoteValue));
        }
        
        if (message.TargetType == "Answer")
        {
            await db.Answers.Where(x => x.Id == message.TargetId)
                .ExecuteUpdateAsync(setter => 
                    setter.SetProperty(x => x.Votes, x => x.Votes + message.VoteValue));
        }
    }
